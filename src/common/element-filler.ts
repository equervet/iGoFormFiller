...
class ElementFiller {
  ...
  private selectRandomiGoRadio(radioId: string, element: HTMLElement): boolean {
    let i = 0;
    const list = [];

    // iGo - handle specific iGo grouping
    var groupElement = element.closest("div.radio-group[role=group]");
    if (groupElement === null)
      return false;
    var elements = groupElement.querySelectorAll("input") as NodeListOf<HTMLInputElement>;

    // only trigger the action on the first radio of the group to avoid uncessary triggers
    if (elements.length <= 0 || radioId !== elements[0].id)
      return true; // assume the action was triggered
  ...
  private getElementName(element: FillableElement): string {
    ...
    // iGo alt-id
    if (this.options.fieldMatchSettings.matchiGoAltId) {
      var altId = (<HTMLInputElement>element).getAttribute("alt_id");
      if (null !== altId) {
        console.log("iGo-FormFiller - attribute alt-id found: " + altId);
        normalizedName += ` ${SanitizeText(altId)}`;
      }
    }
  ...
  private generateDummyDataForCustomField(
    ...
    // iGo
    if (!customField && this.options.fieldMatchSettings.matchiGoRegexpPattern){
      const numberPattern: ICustomField = {
        type: 'number',
      ...
      if (element != null && element.type === "text" && element.getAttribute("pattern") === "\\d*"){
        customField = numberPattern;
      }
    }
    ...
    switch (customField.type) {
      ...
      case 'randomized-list':
        if (customField.list && customField.list.length > 0) {
          return customField.list[this.generator.randomNumber(0, customField.list.length)];
        }
        return '';
  ...
  public fillInputElement(element: HTMLInputElement): void {
    ...
    switch (elementType) {
      ...
      case 'radio':
        debugger;
        if (element.name) {
          var radioButtonTriggered: boolean = false;
          if (this.options.fieldMatchSettings.matchiGoRadioButtonsLogic)
            radioButtonTriggered = this.selectRandomiGoRadio(element.id, element);
          if (!radioButtonTriggered)
      ...
      default:
        if (this.isAnyMatch(element.name.toLowerCase(), this.options.confirmFields)) {
          ...
          element.value = this.trimTextToMaxLenght(element, this.previousValue);
        }
        break;
    }
  ...
  public trimTextToMaxLenght(element: HTMLInputElement, txt: string): string {
    var maxLenAttr = element.getAttribute("maxlength");
    if (this.options.fieldMatchSettings.matchiGoMaxLength && maxLenAttr !== null && !isNaN(parseInt(maxLenAttr))) {
      txt = txt.substring(0, parseInt(maxLenAttr) - 1)
    }
    return txt;
  }
